 ...
 * 包含如下数据项内容：
 * 请求记录唯一编号,姓名,身份证号码,手机号码,授信账户操作代码,循环授信账户编号,授信
 * 担保类型,授信申请时间,授信账户开立时间,授信到期日期,授信额度,授信账户状态,固定账
 * 单日,固定还款日,逾期宽限期,授信账户信息变动时间,设备信息,设备类型,设备MEI/MEID,
...
@Slf4j
@Component
public class X2LoopLoanAccountInfo implements ReportStrategy {
    ...
    @Resource
    private Pipe<Object,Object> stageThree;
    @Resource
    private Pipe<Object,Object> stageFour;
    ...
    public void buildStage() {
        simplePipeline
                ...
                .addPipe(stageThree, executorService)
                .addPipe(stageFour, executorService)
                .init();
    }
}
